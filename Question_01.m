...
R = 1;  %radius
N = 8;  % number of point
angles = flip(linspace(pi/8,2*pi+pi/8,N+1));
...
for start_y = -2.25 : 0.25 : 2.25
    ...
    if start_y == 0
        ...
        sep_start_x = 0.93; % x location for start of sep on rhs of geometry
        sep_rhs = RK_4_2vars(dt,t0,tf,sep_start_x,start_y);
        sep_lhs = RK_4_2vars(dt,t0,tf,start_x,start_y);
...
Uc_i = q*u + U;
Cp_i = 1 - (Uc_i/Uinf).^2;
t2 = [pi/8:0.001:2*pi+pi];
Cp_act = 1-(4*(sin(t2)).^2);
plot(pressure,t2,Cp_act,angles(1:end-1)-pi/8,Cp_i, 'r:o')
