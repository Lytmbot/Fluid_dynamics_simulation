function [ XYT ] = RK_4_2vars_vortex(dt,t0,tf,xi,yi)
...
function [dxdt, dydt] = dxdt_dydt(xp,yp)
    ...
    u_prime = g./(2.*pi).*( atan((x_o_prime-b)./y_o_prime) -...
                            atan((x_o_prime-a)./y_o_prime));                                        
                           
    v_prime = -(g./(4*pi).*(...
        (log( (x_o_prime-b).^2 + y_o_prime.^2)) -...
        (log( (x_o_prime-a).^2 + y_o_prime.^2))));   
